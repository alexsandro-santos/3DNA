...
class Traj3D:
    ...
    def getEval(self) -> float:
        # On ajoute une condition : getAngle() < -0.7 singnifie que nous points sont "relativement alignés" 
        # 2 choix : soit une heuristique pondérée avec k1*angle + k2*distance -> difficile à mettre en oeuvre et n'assure pas de très bons résultats 
        # soit une condition sur l'angle -> problème explore beaucoup moins d'espace.
        if self.getAngle() > -0.7:
            return 100000
        else:
